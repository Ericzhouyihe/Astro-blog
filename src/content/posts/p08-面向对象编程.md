...
每当实例化时, 先自动调用魔术方法__new__(cls, *args, **kwargs), 把要实例化的类对象(即: Student)作为实参传递给形参cls, 并把实例化时传入的其他实参(即: '张三', 28)传递给形参*args, **kwargs, 然后__new__方法根据cls创建出一个对应的实例对象, 并返回该实例对象(即: stu1=该实例对象)
...
- [p07：函数、封包与解包、命名空间与作用域](./p07-函数-封包-解包-命名空间-作用域.md)
